...
class Dot {
  constructor(x, y) {
    ...
    this.friction = 0.97;
    this.gravity = new Vector(0, 0.6); // Adjusted for more reasonable gravity
    this.mass = 1;
  ...
  update(mouse) {
    ...
    // Constrain vertical movement to prevent falling out of the screen
    if (this.pos.y + vel.y >= App.height) {
      this.pos.y = App.height; // Stop at the bottom
    } else {
      this.pos.add(vel);
    }
